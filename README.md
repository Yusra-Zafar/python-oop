...
## 2. OBJECT
- Instance of a class
- Can accessa all the data and methods of its class
- It uses memory, whereas class doesn't 
- It performs the actions defined in the class
```
object = ClassName()
...
**Function** --> is any function, can be accessed by anyone  

**Method**: --> is a function that belongs to a class, can be accessed by the object of that class.

**Magic/special method** -->  Starts and ends with double underscores. An object doesn't call these methods. They get called on a given specific criteria automatically. 
...
**Constructor** --> Magic/Special method that gets called/executed when an object of that class is created. Its control is not given to the user. All the configuration related tasks that should be done automatically when the software opens, should be written inside the constructor.

**self** --> It is the object with which you are working right now. Jis bhi object k saath aap abhi kaam kr rhe ho, wahi self he. 
...
**Getter** --> A method for private data member to fetch the data if needed. 

 **Setter** --> Set the private data member's value if needed but according to my set logic, so that nothing crashes

**Reference Variable** --> In obj = Class(), technically obj is not the object itself, but in fact it is the reference variable that is pointing to the memory location where Class object has been created. 

**Pass by Reference** --> We can pass our object as argument into a function. It will behave exactly the same as any other object in python like int, string, list, tuple etc. And OBJECTS ARE **MUTABLE**.
...
**Collection of Objects**--> We can make a collection of our objects like a list, tuple or dictionary. But not a set because set needs immutable data type and objects are mutable.

**Instance variable** --> The variable defined in constructor method for which every object's value is different. Always initialized inside the class. Access through self (object)

**Static/Class variable** --> A variable whose value is same for all objects of the class. Opposite of instance variable. Always initialized outside the class. Access through class.

**Static Method** --> When dealing with static variables, we use static methods. It doesn't need an object (self) to be passed as parameter because it is not accessing any instance variable, rather it is using only the static/class variables through class name. 

**Types of Relationship** --> Aggregation & Inheritance
